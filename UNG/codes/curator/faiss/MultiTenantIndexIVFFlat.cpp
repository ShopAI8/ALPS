...
void MultiTenantIndexIVFFlat::add_core(
    ...
    FAISS_NAVIX_THROW_IF_NOT(!by_residual);
    assert(invlists);
...
InvertedListScanner* get_InvertedListScanner1(
    ...
    if (ivf->metric_type == METRIC_INNER_PRODUCT) {
        ...
        FAISS_NAVIX_THROW_MSG("metric type not supported");
    }
}
...
void MultiTenantIndexIVFFlat::search_with_filter(
    ...
    if (params) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_MSG(ivf_params, "IndexIVF params have incorrect type");
    }
    const size_t nprobe =
