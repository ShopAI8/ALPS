...
// 16 (was 10): real datasets contain large groups of duplicate vectors that
// k-means cannot separate, so they all sink into one leaf at
// CURATOR_MAX_TREE_DEPTH (Music: largest group = 5742 > 2^10). The local vid
// would then overflow into the tree-path bits of the encoded vid, corrupting
// child indices and crashing grant_access/split_short_list. 16 bits caps the
// tree depth at (64-16)/6 = 8, still far deeper than any practical tree.
constexpr size_t CURATOR_MAX_LEAF_SIZE_LOG2 = 16;
...
struct IdAllocator {
    ...
    ExtLabel allocate_reserved_label() {
        ...
        FAISS_NAVIX_THROW_MSG("No available reserved label");
    }
    ...
    const ExtLabel get_label(IntLabel id) const {
        if (id >= id_to_label.size() || id_to_label[id] == INVALID_ID) {
            FAISS_NAVIX_THROW_MSG("id does not exist");
        }
...
struct IdMapping {
    ...
    void remove_mapping(ExtLabel label) {
        FAISS_NAVIX_THROW_IF_NOT_MSG(has_label(label), "label does not exist");
    ...
    const ExtLabel get_label(IntLabel id) const {
        FAISS_NAVIX_THROW_IF_NOT_MSG(has_id(id), "id does not exist");
...
struct RunningMean {
    ...
    void remove(double x) {
        FAISS_NAVIX_ASSERT_MSG(n > 0, "no elements to remove");
...
struct SortedList {
    ...
    void erase(const T& item) {
        auto it = std::lower_bound(data.begin(), data.end(), item);
        FAISS_NAVIX_ASSERT_MSG(it != data.end() && *it == item, "item not found");
        data.erase(it);
    }
...
struct MultiTenantIndexIVFHierarchical : MultiTenantIndex {
    ...
    void add_vector(idx_t n, const float* x) override {
        FAISS_NAVIX_THROW_MSG("add_vector not supported");
    }

    void reset() override {
        FAISS_NAVIX_THROW_MSG("reset not supported");
    }
    ...
    TreeNode* find_nearest_vacant_leaf(const float* x);
