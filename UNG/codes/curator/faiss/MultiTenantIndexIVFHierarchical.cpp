...
void IdAllocator<ExtLabel, IntLabel>::free_id(ExtLabel label) {
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(it != label_to_id.end(), "label does not exist");
...
void MultiTenantIndexIVFHierarchical::add_vector_with_ids(
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (leaf->vector_indices.size() >= CURATOR_MAX_LEAF_SIZE) {
            // The vid encoding only reserves CURATOR_MAX_LEAF_SIZE_LOG2 bits
            // for the within-leaf index; overflowing it would corrupt the
            // tree-path bits. Spill to the nearest leaf that still has room.
            leaf = find_nearest_vacant_leaf(xi);
            FAISS_NAVIX_THROW_IF_NOT_MSG(
                    leaf != nullptr,
                    "all leaf nodes are full; increase CURATOR_MAX_LEAF_SIZE_LOG2");
            n_spilled++;
        }
        auto offset = sizeof(int_vid_t) * 8 -
    ...
    if (n_spilled > 0) {
        printf("[Curator] add_vector_with_ids: %zu / %zu vectors spilled to "
               "non-nearest leaves (leaf capacity %zu)\n",
               n_spilled,
               (size_t)n,
               CURATOR_MAX_LEAF_SIZE);
    }
...
bool MultiTenantIndexIVFHierarchical::remove_vector(idx_t label) {
    FAISS_NAVIX_THROW_MSG("remove_vector is not supported");
}
...
void MultiTenantIndexIVFHierarchical::search_one(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(search_ef > 0, "search_ef must be greater than 0");
...
TreeNode* MultiTenantIndexIVFHierarchical::find_nearest_vacant_leaf(
        const float* x) {
    // Best-first search over the tree for the nearest leaf that still has
    // capacity in the vid encoding. Only used when the greedy leaf is full,
    // which happens for large groups of (near-)duplicate vectors that k-means
    // cannot separate (the group then sinks into a single leaf at
    // CURATOR_MAX_TREE_DEPTH).
    using QEntry = std::pair<float, TreeNode*>;
    std::priority_queue<QEntry, std::vector<QEntry>, std::greater<QEntry>> pq;
    pq.emplace(0.0f, tree_root);
    ...
    while (!pq.empty()) {
        ...
        if (node->children.empty()) {
            if (node->vector_indices.size() < CURATOR_MAX_LEAF_SIZE) {
                return node;
            }
            continue;
        }
        ...
        for (TreeNode* child : node->children) {
            pq.emplace(fvec_L2sqr(x, child->centroid, d), child);
        }
    }
...
ext_lid_t MultiTenantIndexIVFHierarchical::get_filter_label(const std::string& filter_key) const {
    ...
    if (it == filter_to_label.end()) {
        FAISS_NAVIX_THROW_FMT("Filter key '%s' not found. Make sure to call build_index_for_filter first.", filter_key.c_str());
    }
...
void search_temp_index(
    ...
    if (index->beam_size == 0) {
        FAISS_NAVIX_THROW_MSG("beam_size must be greater than 0");
    }
