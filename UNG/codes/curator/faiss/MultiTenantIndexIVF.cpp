...
void MultiTenantIndexIVF::add_core(
    ...
    FAISS_NAVIX_THROW_IF_NOT(coarse_idx);
    FAISS_NAVIX_THROW_IF_NOT(is_trained);
...
void MultiTenantIndexIVF::search(
    ...
    if (params_in) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_MSG(params, "IndexIVF params have incorrect type");
    }
    const size_t nprobe =
            std::min(nlist, params ? params->nprobe : this->nprobe);
    FAISS_NAVIX_THROW_IF_NOT(nprobe > 0);
...
#pragma omp single
            ...
            FAISS_NAVIX_THROW_FMT("parallel_mode %d not supported\n", pmode);
        }
    } // parallel section

    if (interrupt) {
        if (!exception_string.empty()) {
            FAISS_NAVIX_THROW_FMT(
                    "search interrupted with: %s", exception_string.c_str());
        } else {
            FAISS_NAVIX_THROW_MSG("computation interrupted");
        }
    }
...
#pragma omp parallel if (do_parallel) reduction(+ : nlistv, ndis)
    {
        ...
        std::unique_ptr<InvertedListScanner> scanner(
                get_InvertedListScanner(store_pairs, &mt_sel));
        FAISS_NAVIX_THROW_IF_NOT(scanner.get());
...
void MultiTenantIndexIVF::reconstruct_n(idx_t i0, idx_t ni, float* recons)
        const {
    FAISS_NAVIX_THROW_IF_NOT(ni == 0 || (i0 >= 0 && i0 + ni <= ntotal));
...
void MultiTenantIndexIVF::reconstruct_from_offset(
        ...
        int64_t /*offset*/,
        float* /*recons*/) const {
    FAISS_NAVIX_THROW_MSG("reconstruct_from_offset not implemented");
}
...
void MultiTenantIndexIVF::update_vectors(
        ...
        const float* x,
        tid_t tid) {
    FAISS_NAVIX_THROW_MSG("update_vectors not implemented");
}
...
void MultiTenantIndexIVF::check_compatible_for_merge(
    ...
    FAISS_NAVIX_THROW_IF_NOT(other);
    FAISS_NAVIX_THROW_IF_NOT(other->d == d);
    FAISS_NAVIX_THROW_IF_NOT(other->nlist == nlist);
    FAISS_NAVIX_THROW_IF_NOT(quantizer->ntotal == other->quantizer->ntotal);
    FAISS_NAVIX_THROW_IF_NOT(other->code_size == code_size);
    FAISS_NAVIX_THROW_IF_NOT_MSG(
...
void MultiTenantIndexIVF::replace_invlists(InvertedLists* il, bool own) {
    ...
    // FAISS_NAVIX_THROW_IF_NOT (ntotal == 0);
    if (il) {
        FAISS_NAVIX_THROW_IF_NOT(il->nlist == nlist);
        FAISS_NAVIX_THROW_IF_NOT(
