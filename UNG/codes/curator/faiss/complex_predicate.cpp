...
State make_state(Type type, bool concretize, const Buffer& list) {
    switch (type) {
        case Type::SOME: {
            if (!concretize) {
                FAISS_NAVIX_THROW_MSG("Should not manually create SOME states in abstract mode");
            } else if (list.empty()) {
                return STATE_NONE;
            } else {
        ...
        case Type::MOST: {
            if (!concretize) {
                FAISS_NAVIX_THROW_MSG("Should not manually create MOST states in abstract mode");
            } else if (list.empty()) {
                return STATE_ALL;
            } else {
        ...
        default:
            FAISS_NAVIX_THROW_MSG("Should not manually create states of type ALL, NONE, or UNKNOWN");
    }
}
